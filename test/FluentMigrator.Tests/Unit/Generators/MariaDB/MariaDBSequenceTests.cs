...
namespace FluentMigrator.Tests.Unit.Generators.MariaDB
{
    ...
    [Category("Sequence")]
    [TestFixture]
    public class MariaDBSequenceTests : BaseSequenceTests
    {
        private MariaDBGenerator Generator;
        ...
        [Test]
        public void CanCreateSequenceWithType()
        {
            var expression = GeneratorTestHelper.GetCreateSequenceExpression();
            expression.Sequence.Type = DbType.Int32;

            var result = Generator.Generate(expression);
            result.ShouldBe("CREATE SEQUENCE `Sequence` AS INT INCREMENT 2 MINVALUE 0 MAXVALUE 100 START WITH 2 CACHE 10 CYCLE");
        }
